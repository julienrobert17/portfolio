...
import { chargerDrawSvg, type registerGsap } from './gsap'
...
/**
 * Trace un dessin au trait (DrawSVG, chargé à la demande) en 1,6 s à
 * l'entrée dans le viewport, puis fait apparaître les cotes (paths
 * `data-cote` et libellés HTML `[data-cote-label]` du même conteneur).
 * Retourne une fonction d'annulation : le tracé est créé hors du contexte
 * GSAP du composant, il faut le tuer soi-même au démontage. Sans JavaScript,
 * le dessin est complet.
 */
export function tracerDessin(gsap: Gsap, svg: SVGSVGElement, options: { trigger?: Element; duree?: number; conteneur?: Element } = {}): () => void {
  let annule = false
